...
This benchmark runs without Crodox. This guide adds Crodox as a third arm: the same tasks, the same prompts, but Arm B is a workbench that Crodox generated from your own template instead of a hand-built slice. It also explains how the template's patches work and how to extend them.
...
* Record the arm as `C_crodox` in `results.csv`. `analyze.py` reads `A_full_repo` and `B_workbench`; to compare three arms, run it twice with a filtered CSV, or extend `ARM_B` in the script to a list. Extending it is a ten-line change; do it once the first Crodox rows exist.
...
4. `momentDependency` (component that imports `moment`): adds `"moment"` to `package.json`. No benchmark component needs it; it is there as the worked example of a conditional dependency patch.
5. `serviceProvider` (service): import plus `providers`.
6. `moduleImport` (module): import plus `imports`.
...
Verified here: the template installs, builds and passes its own spec. With the placeholders resolved by hand exactly as the patches specify, each of the five components was dropped into the template, the reference solution applied, and the hidden specs run: all pass, all five workbenches build.
